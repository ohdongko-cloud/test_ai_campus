...
// 통과 시 signup_token(15분 유효) 반환.
export async function POST(req: NextRequest) {
  const ip = getClientIp(req);
  const rl = await checkRateLimit('signup-verify', ip, 10, '5 m');
  ...
  const v = rows[0];
  if (!v) {
    await logAuth({ type: 'signup_verify_failure', email, success: false, req, detail: 'no-pending' });
  ...
  if (new Date(v.expires_at) < new Date()) {
    ...
    await logAuth({ type: 'signup_verify_failure', email, success: false, req, detail: 'expired' });
    return NextResponse.json({ error: '코드가 만료되었습니다. 다시 요청해주세요.' }, { status: 410 });
  }

  if (v.attempts >= MAX_ATTEMPTS) {
    await sql`UPDATE email_verifications SET consumed = true WHERE id = ${v.id}`;
    await logAuth({ type: 'signup_verify_failure', email, success: false, req, detail: 'too-many-attempts' });
  ...
  if (candidateHash !== v.code_hash) {
    ...
    await logAuth({ type: 'signup_verify_failure', email, success: false, req, detail: 'mismatch' });
    return NextResponse.json({ error: '인증 코드가 일치하지 않습니다.' }, { status: 401 });
  }

  await sql`UPDATE email_verifications SET consumed = true WHERE id = ${v.id}`;
  const signupToken = await signSignupToken(email, 15 * 60);
  await logAuth({ type: 'signup_verify_success', email, success: true, req });
