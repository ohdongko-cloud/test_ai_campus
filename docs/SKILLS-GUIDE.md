...
기획 → 구현 → 배포를 **표준화**하는 6개 스킬 + 지침(`CLAUDE.md`) + 훅(가드레일) 사용법.
...
| 층 | 무엇 | 위치 |
|---|---|---|
| **스킬**(호출형 절차) | `/prd-new` `/prd-review` `/prd-commit` `/security-gate` `/ship` `/prd-flow` | `.claude/skills/*/SKILL.md` |
...
- ⚠️ **최초 1회**: `.claude/skills/`가 새로 생겼으므로 **Claude Code를 재시작**해야 스킬이 인식된다.
...
```
/prd-new  →  /prd-review  →  (구현)  →  /prd-commit  →  /security-gate  →  /ship
            [게이트① 품질]                              [게이트② 보안]
```
